...
function acquia_install_tasks_alter(&$tasks, $install_state) {
  ...
  // Insert welcome page after load profile step. This will usually be skipped
  // except for cases where a request with no user-agent header hits the install
  // page. If an actual user hits, it, they can click thorugh. This trick solves
  // the problem of the install starting in the background for some stack
  // installers. Update: webmatrix now sends a user-agent header so we don't check it
  $run = INSTALL_TASK_SKIP;
  if (empty($install_state['parameters']['welcomed'])) {
    $run = INSTALL_TASK_RUN_IF_NOT_COMPLETED;
  }
  $welcome_page = array(
    'acquia_welcome_page' => array(
      'display_name' => st('Start install'),
      'display' => TRUE,
      'type' => 'normal',
      'run' => $run,
    ),
  ...
  $tasks = array_merge(array_slice($tasks, 0, 3), $welcome_page, array_slice($tasks, 3));
}

/**
 * Returns page welcome contents
 */
function acquia_welcome_page($install_state) {
  drupal_set_title('Welcome');
  // l() is not available here
  return '<h3>Welcome to Acquia Drupal installation</h3><a href="install.php?profile=acquia&locale=en&welcomed=1">Click here to continue</a>';
}
...
function _acquia_profile_create_node($filename, $page_type) {
  ...
  $link_regexp = "/(\[\[)([\/?\w+\-*]+)(\]\])/";
  $contents = preg_replace_callback($link_regexp, '_acquia_profile_url', $contents);
  $node = new stdClass();
  $node->title = $title;
 ...
 *
 * Calls url() on matched element 2.
 *
 * @param $matches
 ...
 *  The local url.
 */
function _acquia_profile_url($matches) {
  $options = array();
  $options['alias'] = TRUE;
  return check_url(url($matches[2], $options));
}
